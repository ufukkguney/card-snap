...
public static class MouseUtilities
{
    ...
    public static Vector3 GetMouseWorldPosition(Camera camera, float zDepth)
    {
    ...
    public static Vector3 GetValidDropPosition2D(Camera camera, LayerMask groundLayer, Vector3 fallbackPosition, float raycastDistance = 20f, float heightOffset = 0.1f)
    {
        if (camera == null) return fallbackPosition;
        
        Vector3 mouseWorldPos = GetMouseWorldPosition(camera, 0f);
        
        RaycastHit2D hit = Physics2D.Raycast(mouseWorldPos + Vector3.up * 10f, Vector2.down, raycastDistance, groundLayer);
        
        if (hit.collider != null)
        {
            return new Vector3(hit.point.x, hit.point.y + heightOffset, mouseWorldPos.z);
        }
